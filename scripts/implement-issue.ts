...
export const CONFIG_DIR = join(SCRIPT_DIR, "polytoken-config");
...
export function plannedCommands(issue: IssueReference, repoRoot: string): string[][] {
  ...
  const configDir = resolve(repoRoot, "scripts", "polytoken-config");
  return [["jj", "workspace", "add", ws, "--name", `issue-${issue.number}`, "--revision", "main"], ["bun", "install"], ["polytoken", "--config-dir", configDir, "new", "--no-attach"], ["zellij", "action", "new-tab", "--cwd", ws, "--", "polytoken", "attach", "<session_id>"]];
}
