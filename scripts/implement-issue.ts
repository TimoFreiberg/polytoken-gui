...
export function plannedCommands(issue: IssueReference, repoRoot: string): string[][] {
  ...
  return [["jj", "workspace", "add", ws, "--name", `issue-${issue.number}`, "--revision", "main"], ["bun", "install"], ["polytoken", "new", "--no-attach"], ["zellij", "action", "new-tab", "--block-until-exit", "--cwd", ws, "--", "polytoken", "attach", "<session_id>"]];
}
...
export async function runLauncher(args: string[], options: LauncherOptions = {}): Promise<void> {
  ...
  const context = await mkdtemp(join(repoRoot, ".pantoken-issue-context-")); let claimed = false; let workspace = resolve(repoRoot, ".workspaces", `pantoken-issue-${issue.number}`); let daemonPid: string | undefined;
