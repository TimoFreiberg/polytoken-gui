...
// The @-mention file-index + fallback-query fd helpers now live in the shared
// `file-search.ts` module (used by both the pi and polytoken drivers). The
// `escapeFdRegex`/`buildFdPathQuery`/`baseFdArgs`/`runFd`/`parseFdLines`/caps below
// were extracted verbatim; pi-driver imports the two entry points it needs.
